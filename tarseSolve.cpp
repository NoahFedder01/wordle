...
This was used a lot when making the original txt files, and is
now exclusively used for populating 'small', the possible
answers Word vector in main
*/
...
vector<Word> initializeWithRatings(string filename) {
    ifstream file(filename);  // Open file
    if (!file) {                              // Handle errors opening file
        cerr << "Given parameters are impossible with tarse, or the answer possibilities list is outdated." << endl; 
    }
...
*/
void populateRatingAndFile(vector<Word>& myWords, vector<Word>& small, string filename) {
    Word& best = myWords[0];
...
void crappySort(vector<Word> vec, string filename) {
    ofstream outputFile(filename);
    while (vec.size() != 0) {
        int temp = 0;
        for (int i = 0; i < vec.size(); i++) {
...
void createTarseFiles(int startIndex) {
    string filename = "";
    vector<int> myVec(5,0);
    vector<Word> myWords;
    vector<Word> small;
    char params[5];
    Word tarse = Word("tarse", 5.17842);

    for (int i = 0; i < startIndex; i++) {
        incrementBaseThree(myVec);
    }

    for (int i = startIndex; i < 243; i++) {
        myWords = initializeWithRatings("efficientSorted.txt");
        small = init("wordle-answers-alphabetical.txt");
        for (int j = 0; j < 5; j++) {
            if (myVec[j] == 0) {
                filename += 'N';
                params[j] = 'N';
        ...
        filename += ".txt";
        filename = "tarseFiles/" + filename;
        cout << "Creating " << filename << endl << endl;

        purge(small, tarse, params);

        if (small.empty()) {        // IMPORTANT: if a set of parameters is incompatible with tarse, the .txt DOES NOT EXIST
            incrementBaseThree(myVec);
            filename = "";
            continue;
        }

        if (small.size() == 1) {    // IMPORTANT: if the first guess leaves only one possibility, it is the only word in the
            small[0].rating = -1;   // corresponding .txt file and its rating is -1
            crappySort(small, filename);
            incrementBaseThree(myVec);
            filename = "";
            continue;
        }
...
int main() {
    ...
    while (timer < 6) {
        ...
        if (timer == 1) {           //  First time, guaranteed use of tarse
            cout << endl << "The top words and their scores are:" << endl;
            for (int i = 0; i < 5; i++) {
                cout << myWords[i].word << " " << myWords[i].rating << endl;
            }
            cout << endl << "This is tarse mode, so your first choice must be tarse." << endl << endl;
            ...
            }       // End recieving parameters

            for (int i = 0; i < 5; i++) {       // Make parameters usable in purge()
                params[i] = param[i];
            }

            purge(small, myWords[0], params);   // Remove words incompatible with tarse and given parameters

            param = "tarseFiles/" + param + ".txt";     // Turn param into filename

            myWords = initializeWithRatings(param);     // Retrieve case specific file

            if (myWords[0].rating == -1) {
                cout << "The answer is " << myWords[0].word << endl << endl;
                return 0;
            }

        }       // End first run through
            
        if (timer != 1) {       // Toggle from elimination mode to solve mode on move 3
            for (int i = 0; i < myWords.size(); i++) {
        ...
        cout << "The top words and their scores are:" << endl;
        for (int i = 0; i < max; i++) {
