...
void populateRatingAndFile(vector<Word>& myWords, vector<Word>& small) {
    Word& best = myWords[0];
    ofstream outputFile("efficientUnsorted.txt");
    for (int i = 0; i < myWords.size(); i++) {
        Word& temp = myWords[i];
...
void crappySort(vector<Word> vec) {
    ofstream outputFile("efficientSorted.txt");
    while (vec.size() != 0) {
        int temp = 0;
        for (int i = 0; i < vec.size(); i++) {
...
int main() {
    vector<Word> myWords = initializeWithRatings();      // Get vector
    ...
    while (timer <= 6) {
        ...
        if (myWords.size() == 1) {
            cout << endl << "The answer is " << myWords[0].word << endl << endl;
            return 0;
        }
        ...
        purge(myWords, temp, params);

        populateRating(myWords);
